...
const ScatterLabel = (props: ScatterLabelProps) => {
    ...
    // Get name from value or payload
...
export function CompetitorsOverview() {
    ...
    // Find current clinic data
    const currentClinic = mapData.find(d => d.isCurrent);
    const currentPosition = currentClinic?.x || 0;
    const currentScore = currentClinic?.y || 0;

    // Calculate position status
    const getPositionStatus = (position: number) => {
      if (position <= 3) return { label: 'Top 3', color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-300' };
      if (position <= 10) return { label: 'Top 10', color: 'text-orange-600', bg: 'bg-orange-50 border-orange-300' };
      return { label: 'Below Top 10', color: 'text-red-600', bg: 'bg-red-50 border-red-300' };
    };

    const positionStatus = getPositionStatus(currentPosition);

    // Calculate gap analysis
    const topCompetitors = mapData
      .filter(d => !d.isCurrent)
      .sort((a, b) => (b.y || 0) - (a.y || 0))
      .slice(0, 3);

    const gapAnalysis = topCompetitors.map(comp => ({
      name: comp.name,
      score: comp.y || 0,
      position: comp.x || 0,
      scoreGap: (comp.y || 0) - currentScore,
      positionGap: currentPosition - (comp.x || 0),
    }));

    return (
        <div className="space-y-6 pb-20 animate-in fade-in duration-700">
            {/* Hero Summary Dashboard */}
            <BentoCard className={cn('border-2 relative overflow-hidden bg-white', positionStatus.bg)}>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm font-medium text-slate-700">
                                Your market position and competitive analysis
                            </p>
                        </div>
                        <div className="text-center">
                            <div className={cn('text-5xl font-black italic tracking-tighter mb-1', positionStatus.color)}>
                                {currentPosition.toFixed(1)}
                            </div>
                            <div className="text-sm font-bold text-slate-600">
                                Position
                            </div>
                            <div className={cn('text-xs font-bold mt-2 px-3 py-1 rounded-full', positionStatus.bg, positionStatus.color)}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
                        <div className="space-y-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">ClinicAI Score</span>
                                <span className="text-xs font-black text-slate-900">{currentScore.toFixed(1)}%</span>
                            </div>
                            <div className="h-3 w-full bg-slate-200 rounded-full overflow-hidden relative border border-slate-300">
                                <div
                                    className={cn(
                                        ...
                                        currentScore >= 90 ? 'bg-emerald-600' : currentScore >= 50 ? 'bg-orange-500' : 'bg-red-600'
                                    )}
                                    style={{ width: `${currentScore}%`, minWidth: currentScore > 0 ? '4px' : '0' }}
                                />
                            </div>
                        </div>
                        <div className="space-y-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Market Position</span>
                                <span className="text-xs font-black text-slate-900">{currentPosition.toFixed(1)}</span>
                            </div>
                            <div className="h-3 w-full bg-slate-200 rounded-full overflow-hidden relative border border-slate-300">
                                <div
                                    className={cn(
                                        'transition-all duration-500 ease-out rounded-full shadow-sm h-full',
                                        currentPosition <= 3 ? 'bg-emerald-600' : currentPosition <= 10 ? 'bg-orange-500' : 'bg-red-600'
                                    )}
                                    style={{ width: `${Math.max(0, 100 - (currentPosition - 1) * 10)}%`, minWidth: '4px' }}
                                />
                            </div>
                        </div>
                        <div className="space-y-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Competitors</span>
            ...
            {gapAnalysis.length > 0 && (
                <BentoCard title="Gap Analysis" subtitle="Comparison with top competitors">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {gapAnalysis.map((gap, idx) => (
                            <div key={idx} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center justify-between mb-3">
                                    <div>
                                        <div className="text-sm font-bold text-slate-900">{gap.name}</div>
                                        <div className="text-xs text-slate-500">Position: {gap.position.toFixed(1)}</div>
                                    </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium text-slate-600">Score Gap</span>
                                        <span className={cn(
                                            'text-sm font-black',
                                            gap.scoreGap > 0 ? 'text-red-600' : 'text-emerald-600'
                                        )}>
                                            {gap.scoreGap > 0 ? `-${gap.scoreGap.toFixed(1)}` : `+${Math.abs(gap.scoreGap).toFixed(1)}`} pts
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium text-slate-600">Position Gap</span>
                                        <span className={cn(
                                            'text-sm font-black',
                                            gap.positionGap > 0 ? 'text-red-600' : 'text-emerald-600'
                                        )}>
                                            {gap.positionGap > 0 ? `-${gap.positionGap.toFixed(1)}` : `+${Math.abs(gap.positionGap).toFixed(1)}`} pos
                                        </span>
                                    </div>
                                    <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden border border-slate-300 mt-2">
                                        <div
                                            className={cn(
                                                'h-full transition-all duration-500 ease-out rounded-full',
                                                gap.scoreGap > 0 ? 'bg-red-600' : 'bg-emerald-600'
                                            )}
                                            style={{ width: `${Math.min(100, Math.abs(gap.scoreGap))}%`, minWidth: '4px' }}
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <ScatterChart margin={{ top: 20, right: 120, bottom: 20, left: -20 }}>
                                ...
                                <Scatter name="Clinics" data={mapData}>
                                    {mapData.map((entry, index) => (
                                        <Cell
                                            key={`cell-${index}`}
                                            fill={entry.isCurrent ? TOKENS.colors.you : (entry.color || TOKENS.colors.marketAvg)}
