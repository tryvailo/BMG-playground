#!/bin/bash
# Скрипт для пакетного исправления ошибок с минимальным количеством билдов

set -e

echo "🚀 Начало пакетного исправления ошибок..."
echo ""

# Шаг 1: Анализ
echo "📊 Шаг 1: Анализ текущего состояния..."
TOTAL_ERRORS=$(pnpm --filter web lint 2>&1 | grep -c "error" || echo "0")
echo "   Всего ошибок: $TOTAL_ERRORS"
echo ""

# Шаг 2: Автоматическое исправление
echo "🔧 Шаг 2: Автоматическое исправление..."
pnpm --filter web lint --fix || true
echo "   ✅ Автоматические исправления применены"
echo ""

# Шаг 3: Исправление catch блоков
echo "🔧 Шаг 3: Исправление catch блоков..."
if [ -f "./apps/web/scripts/fix-catch-blocks.sh" ]; then
  ./apps/web/scripts/fix-catch-blocks.sh || true
  echo "   ✅ Catch блоки исправлены"
else
  echo "   ⚠️  Скрипт fix-catch-blocks.sh не найден"
fi
echo ""

# Шаг 4: Проверка результата
echo "📊 Шаг 4: Проверка результата..."
rm -rf .eslintcache apps/web/.eslintcache 2>/dev/null || true
NEW_ERRORS=$(pnpm --filter web lint --no-cache 2>&1 | grep -c "error" || echo "0")
FIXED=$((TOTAL_ERRORS - NEW_ERRORS))

echo "   Было ошибок: $TOTAL_ERRORS"
echo "   Стало ошибок: $NEW_ERRORS"
echo "   Исправлено: $FIXED"
echo ""

# Шаг 5: Рекомендации
if [ "$NEW_ERRORS" -lt 100 ]; then
  echo "✅ Рекомендация: Можно коммитить все сразу"
  echo "   Осталось ошибок: $NEW_ERRORS (< 100)"
elif [ "$NEW_ERRORS" -lt 200 ]; then
  echo "⚠️  Рекомендация: Разбить на 2-3 коммита"
  echo "   Осталось ошибок: $NEW_ERRORS (100-200)"
else
  echo "🔴 Рекомендация: Разбить на несколько коммитов"
  echo "   Осталось ошибок: $NEW_ERRORS (> 200)"
  echo ""
  echo "   Топ-10 файлов с ошибками:"
  pnpm --filter web lint --no-cache 2>&1 | grep -E "error" | cut -d: -f1 | sort | uniq -c | sort -rn | head -10
fi

echo ""
echo "📝 Следующие шаги:"
echo "   1. Проверить локально: pnpm --filter web lint --no-cache"
echo "   2. Проверить TypeScript: pnpm --filter web typecheck"
echo "   3. Если все ок - коммитить: git add -A && git commit -m 'fix: исправление ошибок' && git push"








