-- ============================================================
-- Ensure all users have projects with data
-- ============================================================

-- This script ensures that every user in auth.users has:
-- 1. An account in public.accounts
-- 2. A project in public.projects
-- 3. 12 monthly data points in public.weekly_stats for 2025
...
SELECT 
  ...
  COUNT(DISTINCT ws.id) as weekly_stats_count,
  MIN(ws.clinic_ai_score) as min_score,
  MAX(ws.clinic_ai_score) as max_score
FROM auth.users a
