...
Для быстрого тестирования можно временно отключить требование подтверждения email при регистрации. Это позволит новым пользователям сразу входить в кабинет без подтверждения email.
...
4. Найдите раздел **Email Auth**
5. Найдите опцию **"Enable email confirmations"** (или **"Confirm email"**)
6. **Отключите** эту опцию (переключите в положение OFF)
7. Нажмите **Save** (если требуется)

### ⚠️ Важно:

- **Это временная настройка для тестирования!** В продакшене рекомендуется включить подтверждение email для безопасности
- После отключения новые пользователи смогут сразу входить в систему после регистрации
- Существующие пользователи, которые уже подтвердили email, не затронуты
- Код приложения уже готов к работе с отключенным подтверждением (автоматический редирект в кабинет)

### Включение обратно:

Когда тестирование завершено, верните настройку обратно:
1. Включите **"Enable email confirmations"** в Supabase Dashboard
2. Новые пользователи снова будут получать письма с подтверждением
